...
beforeAll(async () => {
  outDir = mkdtempSync(join(tmpdir(), 'zxs-mcp-'));
  const [clientTransport, serverTransport] = InMemoryTransport.createLinkedPair();
  await createServer().connect(serverTransport);
  client = new Client({ name: 'spectral-test', version: '0.0.1' });
  await client.connect(clientTransport);
});

describe('spectral MCP server', () => {
  ...
  it('zx_build assembles and loads symbols', async () => {
    const result = textJson(await call('zx_build', { entry: join(fixtures, 'hello.asm'), outDir }));
  ...
  it('zx_debug: breakpoint by label, continue, step, disasm', async () => {
    ...
    const run = textJson(
      await call('zx_run', { bin: join(outDir, 'hello.bin'), org: '0x8000', frames: 20 })
    );
  ...
  it('zx_state save/load round-trips through .zxstate files', async () => {
    const file = join(outDir, 'mcp.zxstate');
