...
> escribirla. Tiempo estimado: 10 minutos (más lo que tarde sjasmplus).
...
[`@spectral-zx/toolkit`](https://www.npmjs.com/package/@spectral-zx/toolkit),
...
Solo necesitas tres cosas instaladas:

- **Node.js ≥ 20** — compruébalo con `node --version`.
- **sjasmplus en el PATH** — es el *único* binario externo. La ROM del 48K
  ya viaja dentro del paquete (vía `zx-generation`), no la descargas aparte.
...
```bash
npm install -g @spectral-zx/toolkit
```
...
> ¿No quieres instalar nada global? Puedes usar `npx -p @spectral-zx/toolkit zxs <cmd>`
...
Es, con diferencia, el punto donde más gente se atasca en una máquina nueva.
