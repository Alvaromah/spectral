...
export function watchAddCommand(opts: {
  ...
  if (!opts.read && !opts.write) {
    console.error('Specify --read <addr[-addr]> or --write <addr[-addr]>');
    return EXIT.USER_ERROR;
  }
  const meta = loadSessionMeta(opts.state);
...
export function disasmCommand(
  ...
  if (addr === undefined) {
    console.error(`Cannot resolve '${spec}' to an address`);
    return EXIT.USER_ERROR;
  }

  const lines = disassemble((a) => m.memory.read(a), addr, parseInt(opts.count, 10)).map((l) => ({
    addr: sym(l.addr, symbols),
...
export function traceCommand(opts: {
  ...
  const meta = loadSessionMeta(opts.state);
  const symbols = loadSymbols(meta);
  const tracer = new Tracer(parseInt(opts.last, 10));
  ...
  const read = (a: number) => m.memory.read(a);
  const hot = tracer.topHot(parseInt(opts.top, 10)).map((h) => ({
    pc: sym(h.pc, symbols),
    count: h.count,
    text: disassembleOne(read, h.pc).text,
  }));
  const recent = tracer.lastPCs(parseInt(opts.last, 10)).map((pc) => ({
    pc: sym(pc, symbols),
