...
function requireSession(state?: string) {
  ...
  if (!m) {
    console.error('No session state found. Run `zxs run` first.');
  }
  return m;
}
...
export function memReadCommand(
  ...
  const len = Math.min(parseInt(opts.len, 10), 0x10000);
...
export function memWriteCommand(
  ...
  if (!/^([0-9a-fA-F]{2})+$/.test(clean)) {
    console.error(`Invalid hex byte string: '${bytes}' (use e.g. "3E42" or "3e 42")`);
...
export function regsSetCommand(
  ...
  if (name === 'PC') m.cpu.registers.setPC(v);
  else if (REG16.has(name)) m.cpu.registers.set16(name, v);
  else if (REG8.has(name)) m.cpu.registers.set(name, v & 0xff);
  else if (name === 'IM') m.cpu.interruptMode = v & 0x03;
  else {
    console.error(`Unknown register '${reg}' (use A,F,B..L,I,R, AF,BC,DE,HL,SP,IX,IY, PC, IM)`);
    return EXIT.USER_ERROR;
  }
  const statePath = saveSessionMachine(m, opts.state);
  emit(
