...
import { EXIT, emit, parseAddress } from '../output.js';
...
async function runSpec(specPath: string): Promise<TestResult> {
  ...
  const result = await build(resolve(dir, spec.build), { outDir, cwd: dir });
...
export async function testCommand(path: string, opts: { json: boolean }): Promise<number> {
  ...
  if (specs.length === 0) {
    console.error(`No test.json / *.test.json found under ${path}`);
  ...
  return failed.length === 0 ? EXIT.OK : EXIT.USER_ERROR;
}
