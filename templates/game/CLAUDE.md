# __NAME__ — ZX Spectrum 48K game (Spectral toolkit)
...
```bash
zxs build src/main.asm        # JSON errors with line + did-you-mean hints
...
| Works then crashes | `zxs regs` after longer runs: SP drifting? docs/common-bugs.md#stack-drift |
...
`zxs test tests` must pass. When you add a mechanic, add an assertion to
